...
PreviewWindow::PreviewWindow(Katalog::BaseNode* sel_file, QWidget *parent) : QWidget(parent)
{
    ...
    if (sel_file == dynamic_cast<Katalog::Audio*>(sel_file))
    {
        ...
        icon->setPixmap(QPixmap::fromImage(audioimg.scaled(150,300, Qt::KeepAspectRatio, Qt::SmoothTransformation)));
    }
    else if(sel_file == dynamic_cast<Katalog::Photo*>(sel_file))
    {
        QImage photoimg(":/Icons/image-gallery.svg");
        icon->setPixmap(QPixmap::fromImage(photoimg.scaled(150,300, Qt::KeepAspectRatio, Qt::SmoothTransformation)));
    }
    else if(sel_file == dynamic_cast<Katalog::Video*>(sel_file))
    {
        QImage videoimg(":/Icons/video.svg");
        icon->setPixmap(QPixmap::fromImage(videoimg.scaled(150,300, Qt::KeepAspectRatio, Qt::SmoothTransformation)));
    }
    else
    {
        QImage dirimg(":/Icons/folder.svg");
        icon->setPixmap(QPixmap::fromImage(dirimg.scaled(150,300, Qt::KeepAspectRatio, Qt::SmoothTransformation)));
    }
